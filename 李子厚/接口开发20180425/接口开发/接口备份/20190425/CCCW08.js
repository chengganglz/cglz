...
with(imp){ 
	...
	*/
	...
	var strBizDate=PublicBaseUtil.substringByte(socketMsg,"GBK" ,187,14) ; //业务日期  从MES获取
	// strBizDate="20190418";
	...
	var strStockNo=PublicBaseUtil.substringByte(socketMsg,"GBK" ,214,3);;//仓库编码  从MES获取	
	...
	// 重量
	var zl=PublicBaseUtil.substringByte(socketMsg,"GBK" ,177,10);//重量  从MES获取
	if(zl!=null){
		zl=zl.trim();
	}else
	{
	...
	// 作业计划号
	var zyjhh=PublicBaseUtil.substringByte(socketMsg,"GBK" ,201,12);//  从MES获取
	System.out.println("作业计划号:"+zyjhh);
	// 委外方税号
	var wwfsh=PublicBaseUtil.substringByte(socketMsg,"GBK" ,217,20);//  从MES获取
