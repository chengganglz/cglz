...
with(imp){ 
	...
	var socketMsg ="0156CCCW1120190424134727CCCWDTEST2019042401                                    L101DD11                                              0020001200000000200001 ";
	...
	var flag=PublicBaseUtil.substringByte(socketMsg,"GBK",155,1);//操作标志  从MES获取
	if(flag!=null){
		flag = flag.trim();
	}
	...
	if("1".equals(flag)){
		...
		// 合同号
		var hth=PublicBaseUtil.substringByte(socketMsg,"GBK" ,50,10);//  从MES获取
		System.out.println("合同号-hth:"+hth);
		// 合约号
		var hyh=PublicBaseUtil.substringByte(socketMsg,"GBK" ,60,20);//  从MES获取
		...
		var phmc=PublicBaseUtil.substringByte(socketMsg,"GBK",84,50); //牌号名称  从MES获取
		if(phmc!=null){
			phmc=phmc.trim();
		}else{
			phmc="";
		...
		var kd=PublicBaseUtil.substringByte(socketMsg,"GBK" ,140,5);//宽度  从MES获取
		if(kd!=null){
			kd=kd.trim();
		}else
		{
		...
		var hd=PublicBaseUtil.substringByte(socketMsg,"GBK" ,134,6);//厚度  从MES获取
		if(hd!=null){
			hd=hd.trim();
		}else
		{
		...
		var zl=PublicBaseUtil.substringByte(socketMsg,"GBK" ,145,10);//重量  从MES获取
		if(zl!=null){
			zl=zl.trim();
		}else
		{
		...
		var materialGroupNum = PublicBaseUtil.substringByte(socketMsg,"GBK" ,80,4);
		...
		// 成本中心开始
		// 成本中心编码
		var cbzxNo = PublicBaseUtil.substringByte(socketMsg,"GBK" ,80,4);
		// 成本中心实体
		var costcenter = null;
