...
with(imp){ 
	...
	*/
	...
	var strSupNo="1000"; //供应商编码  从MES获取
	...
	if(isSucess){
		if("1".equals(flag)){
			...
			var zl=PublicBaseUtil.substringByte(socketMsg,"GBK" ,161,5);//重量  从MES获取
			if(zl!=null){
				zl=zl.trim();
			}else
			{
			...
			var hetonghao="201904171";//合同号  从MES获取
			var heyuehao="0417";//合约号  从MES获取
	...
	//		    					Context ctxTemp=ctx.
			...
			var ph=PublicBaseUtil.substringByte(socketMsg,"GBK" ,100,50);//牌号  从MES获取
			if(ph!=null){
				ph=ph.trim();
			}
			else
			...
			var phmc=PublicBaseUtil.substringByte(socketMsg,"GBK" ,100,50);//牌号名称  从MES获取
			if(phmc!=null){
				phmc=phmc.trim();
			}
			else
			...
			try
			{
				bgKd=new BigDecimal("12200").divide(new BigDecimal("10"));;
			}catch(  e){
			...
			try
			{
				bgHd=new BigDecimal("810").divide(new BigDecimal("1000"));
			}catch( e){
