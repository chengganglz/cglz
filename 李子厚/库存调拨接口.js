...
with (imp) {
	...
	socketMsg = '0076CCCW1220190425143655CCCWDTEST2019042501         C01                    ';
	/**********************************变量定义 *********************/
	// 约定电文长度
	...
	var dwlx = 0;
	// 业务日期-默认值
	var ywrq = PublicBaseUtil.substringByte(socketMsg, "GBK", 11, 8); //业务日期  从MES获取
	if (ywrq == null) {
		isSucess = false;
	...
	if ("".equals(ywrq)) {
		...
		var isdate = PublicBaseUtil.valiDateTimeWithLongFormat(ywrq);
		if (!isdate) {
			isSucess = false;
	...
	if (isSucess) {
		// 根据卷号获取 单据类型，是否入库
		var getDjlxAndSfrkSql = "SELECT FID,CFDWLX,CFSFRK,CFSCSJ,CFJZH,CFWLID,CFZL FROM  CT_CUS_JZSCCCJL where cfckjh = '" + jh + "'";
		var rows = DbUtil.executeQuery(ctx, getDjlxAndSfrkSql);
		if (rows != null && rows.next()) {
			fid = rows.getString("fid");//记录id
			dwlx = java.lang.Integer.parseInt(rows.getString("CFDWLX"));//电文类型
			isRK = rows.getString("CFSFRK");//是否入库
			ywrq = rows.getDate('CFSCSJ');//生产时间
			jzh = rows.getString('CFJZH');//机组号
			wlid = rows.getString('CFWLID');//物料ID
			zl = new BigDecimal(rows.getString('CFZL'));
			var strBizDate = ywrq;
			System.out.println("电文类型-dwlx:" + dwlx);
			System.out.println("是否入库-isRK:" + isRK);
			System.out.println("业务日期-ywrq:" + ywrq.toString());
				...
				if ("".equals(ykqh) && !"".equals(mbkqh)) {
					...
					switch (dwlx) {
							...
							if (isSucess) {
								...
								if (isSucess) {
									...
									try {
										...
										//bizDate = sdf.format(strBizDate);
										info.setBizDate(strBizDate);
										var c = Calendar.getInstance();
										c.setTime(strBizDate);
									...
									System.out.println("过程-新增生产入库-日期设置完成");
									//单据状态
									...
									if (stock != null) {
										...
										System.out.println("过程-新增生产入库-仓库编码" + stock.getNumber());
									}
									entry.setQty(bgQty);
								...
								if (isSucess) {
									...
									if (pk != null) {
										...
										System.out.println("过程-新增生产入库-成功");
										var strSQLUpdate = "update CT_CUS_JZSCCCJL set CFSFRK=1 where fid='" + fid + "'";
										System.out.println("生产入库接口" + strSQLUpdate);
						...
						case 2: {
							...
							//成本中心
							var costCenter = PublicBaseUtil.getCostCenterOrgUnitInfoByNumber(ctx, temp_cbzx);
							if (costCenter == null) {
								isSucess = false;
								errMsg = errMsg + "成本中心不能为空\r\n";
							}
							...
							var strMaterialNo = "";
							var strUnit = "吨";//单位默认吨
							var strLot = jh;//批次号  从MES获取
							var strStockNo = mbkqh;//仓库编码  从MES获取
							var bgQty = zl;
