...
with(imp) {
	...
	//socketMsg="0265CCCW0220190508140051CCCWD192021039400        2                                                            DD11                                              00350015000000002664000300000C0111202198            31023000017                                          ";
	...
	*/
	...
	if (isSucess) {
		...
		if ("1".equals(flag)) {
			...
			var materialGroupNum = 'H';
			PublicBaseUtil.substringByte(socketMsg, "GBK", 243, 1); //物料组别  从MES获取
			...
			if ("X".equals(materialGroupNum)) {
				...
				materialGroupNum = "01";
				System.out.println("物料编码"+materialGroupNum);
				if (phmc != null) {
					...
					if (rows != null && rows.next()) {
						...
						do {
							...
							try{
								...
								easnumber = new java.text.DecimalFormat("0000").format(i);
							}catch(e){
								isSucess = false;
								errMsg = errMsg + "生成热卷编码报错\r\n";
			...
			if (isSucess) {
				material = SocketFacadeFactory.getLocalInstance(ctx).getMaterialInfo("01", cpnum, materialGroupNum, ph, phmc, bgKd, bgHd, strUnit, "", true);
			...
			var materialGroupNum ="H";// PublicBaseUtil.substringByte(socketMsg,"GBK" ,243,1);//物料组别  从MES获取
			if (materialGroupNum != null) {
				materialGroupNum = materialGroupNum.trim();
			} else {
				materialGroupNum = "";
			...
			if (isSucess) {
				...
				if (material != null) {
					...
					System.out.println("过滤语句"+" select * where parent.baseStatus='4' and   parent.bizType='d8e80652-0106-1000-e000-04c5c0a812202407435C'   and  material.id='" + material.getId().toString() + "' and  lot='" + strLot + "' and parent.CU.id='" + cu.getId().toString() + "'");
					if (purBills != null) {
						if (purBills.size() > 0) {
							//采购退货
