...
with (imp) {
	...
	//socketMsg = "0187CCCW1820190426203150CCCWDL11940003900                                                L101SS400                                             002500125000000000020C02     20190426113722     ";
	...
	var strBizDate = PublicBaseUtil.substringByte(socketMsg, "GBK", 173, 8); //业务日期  从MES获取
	if (strBizDate == null) {
		isSucess = false;
	...
	System.out.println("领料出库单电文号" + dwh);
	var isInTax = true;
	...
	//成本中心
	var centerNum = PublicBaseUtil.substringByte(socketMsg, "GBK", 90, 4);
	centerNum = centerNum.trim();
	var materialGroupNum = "";//物料组别  从MES获取
	if ("L101".equals(centerNum)) {
		centerNum = "007";
		materialGroupNum="09";
	} else if ("L701".equals(centerNum)) {
		centerNum = "008";
		materialGroupNum="19";
	}
	System.out.println("成本中心编码" + centerNum);
	var costCInfo = PublicBaseUtil.getCostCenterOrgUnitInfoByNumber(ctx, centerNum);//成本中心编码  从MES获取
	if (costCInfo == null) {
		isSucess = false;
	...
	var strStockNo = PublicBaseUtil.substringByte(socketMsg, "GBK", 165, 8);//仓库编码  从MES获取
	if(strStockNo==null){
		isSucess=false;
	...
	if("09".equals(materialGroupNum)){
		if("".equals(ph)){
			isSucess=false;
			errMsg=errMsg + "牌号流水不存在\r\n";
		}
	}else if("19".equals(materialGroupNum)){
		if("".equals(ph)){
			isSucess=false;
			errMsg=errMsg + "牌号流水不存在\r\n";
		}
	...
	var costCenterInfo = PublicBaseUtil.getCostCenterOrgUnitInfoByNumber(ctx, centerNum);//成本中心编码  从MES获取
	...
	if (isSucess) {
		...
		info.put("bxNumber", "领用出库电文CCCW18");
		//制单人
